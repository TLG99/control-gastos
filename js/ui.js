// js/ui.js
// Todo lo relacionado con renderizado del DOM y actualizaciones visuales
...
// ── Recalcular todos los totales en pantalla ──
export function recalcular(datos, mesActual) {
  ...
  sv.className   = 'mval ' + (saldo >= 0 ? 'verde' : 'rojo');
  ...
  ss.textContent  = fmt(saldo);
  ss.style.color  = saldo >= 0 ? 'var(--green)' : 'var(--red)';
  ...
  ms.textContent = fmt(saldo);
  ms.className   = 'mc-val ' + (saldo >= 0 ? 'verde' : 'rojo');
  ...
  mbf.style.width      = pctC + '%';
  mbf.style.background = col;
...
export function switchMobTab(tab) {
  ...
  ['fijos', 'varios', 'otros'].forEach(s => {
    document.getElementById('panel-' + s).classList.remove('tab-active');
  });
  ...
  if (tab === 'resumen') {
    document.getElementById('mob-panel-resumen').classList.add('tab-active');
  } else {
